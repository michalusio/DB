...
use db::{DBOperator, ObjectField, Row, Storage};
use uuid::Uuid;
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("10K table nested loop iteration with 100 rows table", |b| {
        b.iter(|| {
...
criterion_group!{
    name = collection_nested_loop;
    config = Criterion::default();
    targets = criterion_benchmark
}
criterion_main!(collection_nested_loop);
